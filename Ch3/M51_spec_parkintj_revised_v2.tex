...
\shorttitle{Heating and cooling variations in M51}
...
\tabletypesize{\small}
\tablecolumns{7}
\tablecaption{Properties of our \emph{Herschel} observations\label{table: herschel_char}}
\tablewidth{0pt}
\tablehead{
...
\colhead{}	   & \colhead{central 80$\arcsec$\tablenotemark{a}} & \colhead{CCM~10\tablenotemark{b}}
 ...
 \enddata
 \tablecomments{Total flux measured within an 80$\arcsec$ aperture centered on the nucleus of M51.  The aperture matches the beam size of the ISO observations.  Only pixels with a 5$\sigma$ detection or better within the aperture were included in calculating the total flux from our \emph{Herschel} maps.}
 ...
 \tablenotetext{c}{This is the same data as presented by \citet{2001A&A...375..566N}; however, an extended source correction has been applied by \citet{2005A&A...441..961K} in calculating the integrated intensity, reducing its value.}
\end{deluxetable}
...
\tablewidth{0pt}
\tablehead{
\colhead{Line} 	            & \colhead{$\mathbf{10^{-4}}$ Line/$F_{\mathrm{TIR}}$\tablenotemark{a}} &
...
\colhead{Region} & \colhead{Observed} & \colhead{$n_{e}$\tablenotemark{a} (cm$^{-3}$)}
    ...
    & \colhead{$[$C\,\textsc{ii}]$^{ionized}_{158}$/[N\,\textsc{ii}]$_{205}$\tablenotemark{b}}
...
\colhead{Region}  & \colhead{Average\tablenotemark{a}} & \colhead{Average\tablenotemark{a}} & \colhead{log($n$/cm$^{-3}$)}
		& \colhead{log$G_{0}$} \\
                  & \colhead{($[\mathrm{CII}]+[\mathrm{OI}]$(63~$\mu$m))/$F_{\mathrm{TIR}}$}
  				  & \colhead{$[\mathrm{CII}]/[\mathrm{OI}]$(63~$\mu$m)} &  & }
 \startdata
...
\tabletypesize{\small}
\tablecolumns{4}
\tablecaption{A comparison of the PDR characteristics measured in M51 to previous studies\label{pdr_comparison}}
\tablewidth{0pt}
\tablehead{
\colhead{Paper} & \colhead{Source(s)} & \colhead{log($n$/cm$^{-3}$)} & \colhead{log$G_{0}$}}
 ...
 \enddata
 \tablerefs{(1) \citet{2001A&A...375..566N} (2) \citet{2001ApJ...561..766M} (3) \citet{1996A&A...315L.117L} (4) \citet{2010A&A...518L..57C} (5) \citet{2002AJ....124..751C} (6) \citet{2005A&A...441..961K} (7) \citet{2012ApJ...747...81C} \textbf{(8) \citet{2011A&A...532A.152M}}}
\end{deluxetable}
...
%The conversion between $U_{\mathrm{min}}$ and $G_{0}$ is $U$ = 0.88$G_{0}$ \citep{2007ApJ...663..866D}, where we only consider $U_{\mathrm{MW}}$ between 910 and 2100~\AA\ to correspond with the energy range of the photons included in $G_{0}$ (6--13~eV) (note that it is possible that the shape of the ISRF in the Milky Way is not appropriate for the inner region of M51, in which case the relative strength of the FUV radiation compared to the rest of the spectrum would change).  Applying this conversion to our derived average $G_{0}$ values, we obtain $U_{\mathrm{min}}$ ranges of 2000-6400, 640-1140, 200-360 and 200-360 for the nucleus, center, arm and interarm regions, respectively.  Our values are much higher than those obtained from the dust SED modelling done by \citet{2012ApJ...755..165M}.  We also note that the higher $G_{0}$ that we measure in the nucleus and center regions compared to the arm and interarm regions is not seen in the results obtained with the dust SED model by \citet{2012ApJ...755..165M}.  This difference is not surprising, since we are modelling line emission from the PDRs themselves, which would be exposed to stronger radiation fields than the ambient global ISRF averaged across the galaxy that is measured by the dust SED modelling.  However, in contrast to what we have found here, the Draine model predicts that only a very small fraction of the dust in M51 is exposed to strong radiation fields with higher values of $U$.  Thus, we believe that in the case of M51, the Draine model results are significantly underestimating the contribution of PDR regions to the total heating in M51.
...
{\it Facility:} \facility{Herschel (PACS, SPIRE)}.
...
{Schinnerer}, E., {Meidt}, S.~E., {Pety}, J., {et~al.} 2013, ArXiv e-prints, 1304.1801
